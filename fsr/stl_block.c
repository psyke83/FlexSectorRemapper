...
static int stl_do_ioctl(u32 volume, u32 partno, u32 cmd, u32 arg)
{
	...
	switch (cmd) 
	{
		...
		case STL_FORMAT:
		{
			...
			FSR_DOWN(&fsr_mutex);
			printk("%s[%d] part_id=%d\n", __func__, __LINE__, part_id);
...
static unsigned int stl_iostat[IO_DIRECTION][MAX_NUMBER_SECTORS + 1];
						...
						int count, int *eof, void *data)
{
	...
	for (i = 0; i < IO_DIRECTION; i++) 
	{
		...
		for (j = 0; j < MAX_NUMBER_SECTORS; j++) 
		{
			...
			len += snprintf(page + len, PAGE_SIZE - len, "[%d: %d] ", j, stl_iostat[i][j]);
		}
...
{
	memset(stl_iostat, 0, sizeof(stl_iostat));
