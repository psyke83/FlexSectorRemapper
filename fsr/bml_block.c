							...
							unsigned cmd, unsigned long arg)
{
	...
	switch (cmd) 
	{
		...
		case BML_GET_MAJOR_NUMBER:
		{
			...
			ret = put_user(major_nr, (char *)arg);
			/* memory error */
			if (ret)
			{
		...
		case BML_GET_MINOR_NUMBER:
		{
			...
			ret = put_user(minor_nr, (char *)arg);		
			/* memory error */
			if (ret)
			{
				ERRPRINTK("copy_to_user error : %x\n", ret);
		...
		case BML_OTP_LOCK:
		{
			...
			if (ret != FSR_BML_SUCCESS)
			{
				ERRPRINTK("FSR: FSR_BML_IOCtl OTP Lock Error\n");
				return -EIO;
			}
...
static unsigned int bml_iostat[IO_DIRECTION][MAX_NUMBER_SECTORS + 1];
							...
							int count, int *eof, void *data)
{
	...
	for (i = 0; i < IO_DIRECTION; i++) 
	{
		...
		for (j = 0; j < MAX_NUMBER_SECTORS; j++) 
		{
			...
			len += snprintf(page + len, PAGE_SIZE - len, "[%d: %d] ", j, bml_iostat[i][j]);
		}
...
{
	memset(bml_iostat, 0, sizeof(bml_iostat));
