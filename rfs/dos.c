...
/* #define MAX_NUMERIC	1024 */ /* move to rfs_fs.h */
...
#define FAT_time(sec, min, hour)	(((hour) << 11) + ((min) << 5) + ((sec)  >> 1))
 ...
 */
static void __convert_date_linux2dos(time_t linux_date, unsigned short *time, unsigned short *date)
{
	...
	/* set the FAT time */
	*time = (unsigned int) (FAT_time(linux_date % SEC_PER_MIN,
			 (linux_date / SEC_PER_MIN) % MIN_PER_HR,
			 (linux_date / SEC_PER_HR) % HR_PER_DAY));

	/* get the days & years */
	day = (unsigned int) ((linux_date - MIN_DATE) / SEC_PER_DAY);
	year = (unsigned int) (day / DAY_PER_YR);
	...
	/* set the FAT date */
	*date = (unsigned short) (FAT_date(day - days[month - 1] + 1, month, year));
}
 ...
 */
static int set_entry_type(struct rfs_dir_entry *ep, unsigned int type)
{
...
static int get_numeric_tail(struct inode *dir, u8 *dos_name) 
{
	...
	bmap = rfs_kmalloc(MAX_NUMERIC >> 3, GFP_KERNEL, NORETRY);
...
/**
 *  Function assigning a FAT time to directory entry' mtime 
 *  by converting a given linux time.
 *  @param ep	directory entry to be modified
 *  @param cur_time	linux time to be converted
 *  @return	none
 */
#ifdef RFS_FOR_2_6
void rfs_set_entry_time(struct rfs_dir_entry *ep, struct timespec cur_time)
{
	...
	__convert_date_linux2dos(tmp_time.tv_sec, &time, &date);
#else
void rfs_set_entry_time(struct rfs_dir_entry *ep, time_t cur_time)
{
	unsigned short time = 0x00, date = 0x21;
	time_t tmp_time = 0;
	...
	__convert_date_linux2dos(tmp_time, &time, &date);
#endif
 ...
 */
unsigned int rfs_entry_type(struct rfs_dir_entry *ep)
{
	if (ep->name[0] == (u8) 0x0)
		return TYPE_UNUSED;
...
#define DOS_time(sec, min, hour)	((sec << 1) + (min * SEC_PER_MIN) + (hour * SEC_PER_HR))
#define days_in_years(year)	(year >> 2) + (year * DAY_PER_YR)
 ...
 */
long rfs_entry_time(unsigned short time, unsigned short date)
{
	long secs, day; 
	...
	secs = (long) (DOS_time((time & 0x1F), 
					((time >> 5) & 0x3F), 
					(time >> 11)));

	/* days since 1.1.70 plus 80's leap day */
	day = (long) (DOS_date((date & 0x1F), 
					(((date >> 5) - 1) & 0xF), 
					(date >> 9))) 
				+ DAY_PER_10YR;
	...
	/* reflect timezone */
	secs += (long) (sys_tz.tz_minuteswest * 60);
 ...
 */
unsigned char rfs_calc_checksum(const u8 *dos_name) 
{
...
int rfs_make_dosname(struct inode *dir, const unsigned char *name, u8 *dosname, unsigned char *mixed, u16 *uname)
{
	...
	uni_len = rfs_convert_dosname(name, dosname, uname, &status, dir->i_sb, TRUE);
	if (uni_len < 0) 
		return uni_len;
	...
	if (get_lossy(status)) {
		if (!test_opt(dir->i_sb, CHECK_NO)) {
			/* make SFN name when CHECK_NO option is disabled. */
			/* SFN name will fill in build_entry() when 
			   CHECK_NO option is enabled. */
 ...
 * @param mode		permission of directory entry
 * @return	none
 */
void 
rfs_init_dir_entry(struct inode *dir, struct rfs_dir_entry *ep, 
		unsigned int type, unsigned int start_clu, 
		const u8 *dosname, unsigned char *mixed, int mode)
{
	SET16(ep->adate, 0);
	SET16(ep->ctime, 0);
	SET16(ep->cdate, 0);
	ep->cmsec = 0;
	rfs_set_entry_time(ep, CURRENT_TIME);
	SET_START_CLUSTER(ep, start_clu);
	...
	rfs_set_uid(ep, rfs_current_fsuid());
	if (dir->i_mode & S_ISGID)
		rfs_set_gid(ep, dir->i_gid);
	else
		rfs_set_gid(ep, rfs_current_fsgid());
#endif
	rfs_set_mode(ep, (mode & S_IRWXUGO));
}
 ...
 * @return	return zero on success, a negative value on failure
 */
int rfs_init_ext_entry(struct rfs_ext_entry *extp, unsigned int type, unsigned int entry_offset, const u16 *uname, unsigned char checksum) 
{
 ...
 */
static inline int __get_root_block(struct super_block *sb, unsigned int *offset, sector_t *block)
{
 ...
 */
struct rfs_dir_entry *get_entry_with_cluster(struct super_block *sb, unsigned int clu, unsigned int entry, struct buffer_head **bh, struct inode *inode)
{
	struct rfs_sb_info *sbi = RFS_SB(sb);
	struct rfs_dir_entry *ep = NULL;
	sector_t block = 0;
	...
	if ((clu != sbi->root_clu) || IS_FAT32(sbi)) {
		...
		block_offset = (off >> sb->s_blocksize_bits) 
				& (sbi->blks_per_clu - 1);
		fat_lock(sb);
		err = rfs_find_cluster(sb, clu, cluster_offset, &prev, &next);
		fat_unlock(sb);
		if (err) {
			if (err == -EFAULT)
			{
				/* It is normal case in rfs_count_subdir */
				return ERR_PTR(err);
			}
			...
			RFS_BUG_CRASH(sb, "can't find entry (%u,%u)\n", 
					clu, entry);
			return ERR_PTR(err);
		}
		...
		err = __get_root_block(sb, &off, &block);
		if (err)
			return ERR_PTR(err);
	}
...
struct rfs_dir_entry *get_entry(struct inode *dir, unsigned int entry, struct buffer_head **res_bh)
{
	...
	if ((RFS_I(dir)->start_clu != sbi->root_clu) || IS_FAT32(sbi)) {
		...
		/* FAT32 or sub directory */
		iblock = (long) (off >> sb->s_blocksize_bits);
 ...
 */
int rfs_get_uname_from_entry (struct inode *dir, unsigned int entry, u16 *uname)
{
	...
	for (i = 0, entry--; (int) entry >= 0; entry--) {
		...
		if (rfs_entry_type(ep) == TYPE_EXTEND) {
...
static int get_long_name(struct inode *dir, unsigned int entry, struct buffer_head **res_bh, struct rfs_dir_entry **ep, u16 *ext_uname)
{
	...
	if (checksum != rfs_calc_checksum((*ep)->name)) {
		/* no extend entries */
		ext_uname[0] = 0x0000;
		DEBUG(DL3, "checksum(%d) != expected(%d)", checksum, 
				rfs_calc_checksum((*ep)->name));
		return 0;
	}
 ...
 * searching with long file name
 */
int rfs_find_entry_long(struct inode *dir, const unsigned char *name, struct buffer_head **bh, unsigned int seek_type) 
{
	struct rfs_dir_entry *ep;
	struct rfs_ext_entry *extp;
	u16 ext_uname[MAX_TOTAL_LENGTH];
	u16 unicode[MAX_TOTAL_LENGTH];
	unsigned char dosname[DOS_NAME_LENGTH + 1];
	unsigned int cpos = 0;
	int nr_slot = 0;
	...
	uni_len = rfs_convert_dosname(name, dosname, unicode, &status, 
				(dir->i_sb), FALSE);
	if (uni_len < 0)
		return uni_len;
	...
	while(1) {
		...
		if (type == TYPE_EXTEND) {
			...
			if (nr_slot == uni_slot) {
				int cmp_len = nr_slot * EXT_UNAME_LENGTH;
				if (!test_opt(dir->i_sb, CHECK_STRICT)) {
					/* case insensitive name cmp */
					if (!rfs_strnicmp(dir->i_sb, ext_uname,
						name)) {
					...
					/* case sensitive name cmp */
					if (!memcmp(ext_uname, unicode,
						cmp_len * sizeof(u16))) {
						if ((seek_type == TYPE_ALL) ||
							(seek_type == type))
							goto found;
 ...
 */
int rfs_find_entry_short(struct inode *dir, const unsigned char *name, struct buffer_head **bh, unsigned int seek_type)
{
	struct rfs_dir_entry *ep;
	...
	ret = rfs_convert_dosname(name, dosname, NULL, NULL, 
			(dir->i_sb), FALSE);
	if (ret < 0)
		return ret;
	...
	while (1) {
		...
		type = rfs_entry_type(ep);
		if ((type == TYPE_FILE) || (type == TYPE_DIR)) {
			if (!strncmp(dosname, (const char*) (ep->name), 
					DOS_NAME_LENGTH)) {
				if (seek_type == TYPE_ALL) {
					goto found;
 ...
 * @param dir	inode for parent
 * @param inode	inode for deleting file
 * @return	zero on success, negative error code on failure	
 */
int rfs_remove_entry(struct inode *dir, struct inode *inode)
{
	struct rfs_dir_entry *ep;
	struct buffer_head *bh = NULL;
	struct log_ENTRY_info lei;
	unsigned int entry = RFS_I(inode)->index;
	unsigned int numof_entries = 0;
	unsigned int i = 0;
	unsigned char undel_buf[MAX_REMOVE_SLOT];
	...
	while (1) {
		...
		if ((i != 0) && (rfs_entry_type(ep) != TYPE_EXTEND))
			break;

		/* sanity check */
		if (i == 0) 
		{
			if (unlikely((rfs_entry_type(ep) != TYPE_FILE) &&
					(rfs_entry_type(ep) != TYPE_DIR))) 
			{
				dump_inode(inode);
				dump_entry(dir->i_sb, ep, bh, 
						RFS_I(inode)->offset, 
						"at %s:%d", __func__, __LINE__);
				RFS_BUG_CRASH(dir->i_sb, "dir entry (%d,%d) is "
						"corrupted\n", 
						RFS_I(dir)->start_clu, entry);
				brelse(bh);
				return -EIO;
			}
			DEBUG(DL3, "name:%s,p_start:%u", ep->name, 
					RFS_I(inode)->p_start_clu);
		}

		BUG_ON(i >= MAX_REMOVE_SLOT);
	...
	lei.entry = entry;
	lei.numof_entries = numof_entries;
	lei.undel_buf = undel_buf;

	if (rfs_log_remove_entry(dir->i_sb, &lei)) 
	{
		brelse(bh);
		return -EIO;
	}

	/* remove entry : set deletion mark */
