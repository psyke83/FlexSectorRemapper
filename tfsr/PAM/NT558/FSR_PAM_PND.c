...
static BOOL Free_Run_Keeping_Flag = FALSE;
...
void HAL_ParFlash_Reads(UINT32 u32Addr, UINT8 *pu8Data, UINT32 u32Size)
{

#ifndef SYNC_READ_BUFFERRAM
    
    unsigned int k;
    unsigned short Data_wTemp;
    UINT32 bufferram;    
    UINT8 * buffer;
    UINT32 count;
    ...
    for (k=0; k<(count/2); k++)
    {        
        *(buffer+2*k) = (unsigned char)(Data_wTemp);
        *(buffer+2*k+1) = (unsigned char)(Data_wTemp>>8);        
    }
...
#else // #ifndef SYNC_READ_BUFFERRAM


    UINT32 count;
           
           
    if ( ((((UINT32)(pu8Data))%32) == 0)  && (((UINT32)(pu8Data)) <= 0xbfffffff) )
    {
//        Nvt_onenand_sync_read_bufferram(u32Addr, pu8Data, u32Size);    // Macoto  ???       
    }
    else
    {
//        Nvt_onenand_sync_read_bufferram(u32Addr, Sync_Read_Buffer, u32Size);    // Macoto  ???
        for (count=0; count < u32Size; count++)
        {        
            *(volatile UINT8*)(pu8Data+count) = *(volatile UINT8*)(Sync_Read_Buffer+count);
        }
    }
        

#endif // #else of #ifndef SYNC_READ_BUFFERRAM

}

void HAL_ParFlash_Writes(UINT32 u32Addr, const UINT8 *pu8Data, UINT32 u32Size)
{
    ...
    bufferram = u32Addr;    
    buffer = (UINT8*)(pu8Data);
    count = u32Size;
    
    for (k=0; k<(count/2); k++)
    {      
      Data_wTemp = (unsigned short)(*((UINT8*)(buffer+2*k))) | (((unsigned short)(*((UINT8*)(buffer+2*k+1))))<<8);  
    }
    if ((count%2) == 1)
    {
        Data_wTemp = (unsigned short)(*(buffer+2*k));  
    }
...
int PNFC_Init( void )
{
    ...
    *(volatile DWORD*)(0xbc040200) = (*(volatile DWORD*)(0xbc040200) & (~(1<<4)))| (1<<5);
...
#ifdef __KERNEL__
{
	...
	//TODO modify this
    RTL_PRINT((TEXT("\r\n[PAM:   ] FSR_PAM_NAND generated on May/19/2011, test_no = 0x%x\r\n"), test_no));                               
