...
#if defined(_WIN32) && defined(FSR_LLD_HANDSHAKE_ERR_INF)
    ...
    do
    {
        ...
        for (nVol = 0; nVol < FSR_MAX_VOLS; nVol++)
        {
            ...
            for (nIdx = 0; nIdx < astPAParm[nVol].nDevsInVol; nIdx++)
            {
...
#if defined(_WIN32) && defined(FSR_LLD_HANDSHAKE_ERR_INF)
                /* Allocate gstOND4kSharedCxt */
                gstOND4kSharedCxt[nPDev] = (OneNAND4kSharedCxt*) FSR_OAM_MallocExt(nMemoryChunkID,
                                                                              sizeof(OneNAND4kSharedCxt),
                                                                              FSR_OAM_SHARED_MEM);
                if (gstOND4kSharedCxt[nPDev] == NULL)
                {
                    FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,
                        (TEXT("[OND:ERR]   %s(nFlag:0x%08x) / %d line\r\n"),
                        __FSR_FUNC__, nFlag, __LINE__));
                    ...
                    FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,
                        (TEXT("            gstOND4kSharedCxt is NULL\r\n")));
                ...
                if (((UINT32) gstOND4kSharedCxt[nPDev] & (0x03)) != 0)
                {
                    FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,
                        (TEXT("[OND:ERR]   %s(nFlag:0x%08x) / %d line\r\n"),
                        __FSR_FUNC__, nFlag, __LINE__));
                    ...
                    FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,
                        (TEXT("            gstOND4kSharedCxt is misaligned:0x%08x\r\n"),
                        pstOND4kShMem));
...
#if !defined (TINY_FSR)
                {
                    /* Initialize  gstOND4kSharedCxt */
                    FSR_OAM_MEMSET((VOID*)gstOND4kSharedCxt[nPDev], 0x00, sizeof(OneNAND4kSharedCxt)); /* initialize shared memory used by LLD                       */

                    for (nDie = 0; nDie < FSR_MAX_DIES; nDie++)
                    {
                        gstOND4kSharedCxt[nPDev]->nHostLLDRe[nDie] = FSR_LLD_SUCCESS; 
                        gstOND4kSharedCxt[nPDev]->nHostLLDPbn[nDie] = FSR_OND_4K_PREOP_ADDRESS_NONE;
                        gstOND4kSharedCxt[nPDev]->nHostLLDPgOffset[nDie] = FSR_OND_4K_PREOP_ADDRESS_NONE;
                    }
                }
#endif
                /* Allocate gstOND4kOpLog */
                gstOND4kOpLog[nPDev] = (OneNAND4kOpLog*) FSR_OAM_MallocExt(nMemoryChunkID,
                                                                       sizeof(OneNAND4kOpLog),
                                                                       FSR_OAM_SHARED_MEM);
                if (gstOND4kOpLog[nPDev] == NULL)
                {
                    FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,
                        (TEXT("[OND:ERR]   %s(nFlag:0x%08x) / %d line\r\n"),
                        __FSR_FUNC__, nFlag, __LINE__));
                ...
                if (((UINT32) gstOND4kOpLog[nPDev] & (0x03)) != 0)
                {
                    FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,
                        (TEXT("[OND:ERR]   %s(nFlag:0x%08x) / %d line\r\n"),
                        __FSR_FUNC__, nFlag, __LINE__));
                    ...
                    FSR_DBZ_RTLMOUT(FSR_DBZ_ERROR,
                        (TEXT("            gstOND4kOpLog is misaligned:0x%08x\r\n"),
                        pstOND4kShMem));
                 ...
                 UINT32       nFlag)
{
    ...
    do
    {
        ...
        if (pMBuf == NULL && pSBuf != NULL)
        {
            ...
            {
                OND_4K_MEMSET_DATARAM((VOID *) pstFOReg->nDataMB00,
                    0xFF,
        ...
        if (pMBuf != NULL && pSBuf == NULL)
        {
            if((nFlag & FSR_LLD_FLAG_BACKUP_DATA) != FSR_LLD_FLAG_BACKUP_DATA)
            {
                OND_4K_MEMSET_DATARAM((VOID *) pstFOReg->nDataSB00,
                    0xFF,
                     ...
                     UINT32       nFlag)
{
    ...
    nTwoSctIdx = ((FSR_SPARE_BUF_SIZE_4KB_PAGE / FSR_OND_4K_SPARE_USER_AREA) >> 2) + pstSrc->nNumOfMetaExt;
        ...
        do
        {
            ...
            if (bEccOn == TRUE32)
            {
                ...
                if (nSctIdx == 2)
                {
                    ...
                    OND_4K_WRITE(*pTgt16++,       nEcc_MSB);
                }
                else
                {
                    nEcc_LSB = OND_4K_READ(*pTgt16); 
                    nEcc_LSB = (nEcc_LSB & 0xFF00) | ((nGenValue >> 8) & 0x00FF);
                    OND_4K_WRITE(*pTgt16++,       nEcc_LSB);
                }
#endif /* #if defined (FSR_LLD_BIG_ENDIAN) */

                /* set address to location for SWEcc area */
                pTgt16 += FSR_OND_4K_SPARE_HW_ECC_AREA -1;
            }
            else /* if (bEccOn == TRUE32) */
            {
                pTgt16 += FSR_OND_4K_SPARE_HW_ECC_AREA;
            }
        }while (--nSctIdx > 0);
                   ...
                   UINT32 nFlag)
{
    ...
    do
    {
            ...
            (pstOND4kSharedCxt->nPrevFSRMode[nDie] & FSR_LLD_FLAG_READ_ONLY))
        {
            ...
            {
#if defined (_WIN32)
                nLLDRe = _CheckErrorCxt(nDev, nDie, pstOND4kCxt, pstOND4kShMem, pstFOReg);
#else
