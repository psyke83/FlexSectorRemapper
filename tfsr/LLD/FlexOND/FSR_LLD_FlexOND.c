...
#define     FSR_FND_PI_LOCK                 (0x3C00)
#define     FSR_FND_PI_NOLOCK               (0xFC00)
              ...
              UINT32       nFlag)
{
    ...
    do
    {
        ...
        if (pMBuf == NULL && pSBuf != NULL)
        {
            ...
            {
                FND_MEMSET_DATARAM((VOID *) pstFOReg->nDataMB00,
                    0xFF,
        ...
        if (pMBuf != NULL && pSBuf == NULL)
        {
            if((nFlag & FSR_LLD_FLAG_BACKUP_DATA) != FSR_LLD_FLAG_BACKUP_DATA)
            {
                FND_MEMSET_DATARAM((VOID *) pstFOReg->nDataSB00,
                    0xFF,
                     ...
                     UINT32       nFlag)
{
    ...
    do
    {
        ...
        pTgt16 += FSR_FND_SPARE_HW_ECC_AREA;
    } while (--nNumOfSct > 0);
              ...
              UINT32 *pByteRet)
{
    ...
    do
    {
        ...
        case FSR_LLD_IOCTL_PI_WRITE:
            ...
            {
                nLLDRe = _WritePI(nDev, nDie, FSR_FND_PI_LOCK | nEndOfSLC);
            }
            else if (nPILockValue == FSR_LLD_IOCTL_UNLOCK_PI)
            {
                nLLDRe = _WritePI(nDev, nDie, FSR_FND_PI_NOLOCK | nEndOfSLC);
            }
            else
            {
                nLLDRe = FSR_LLD_INVALID_PARAM;
        ...
        BOOL32  *pbPILocked)
{
    ...
    do
    {
        ...
        *pnEndOfSLC = (UINT32) (nPIValue & pstFNDCxt->nFBAMask);
        ...
        if (pbPILocked != NULL)
        {
            if ((nPIValue & FSR_FND_PI_LOCK_MASK) ==
                (FSR_FND_PI_LOCK & FSR_FND_PI_LOCK_MASK))
            {
                *pbPILocked = TRUE32;
            }
            else if ((nPIValue & FSR_FND_PI_LOCK_MASK) ==
                (FSR_FND_PI_NOLOCK & FSR_FND_PI_LOCK_MASK))
            {
                *pbPILocked = FALSE32;
            }
            else
