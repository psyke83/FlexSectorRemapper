...
UINT32 NFC_GetECCBits(UINT32 subpage_cnt) ;
UINT32 NFC_ReadPage_FreeRun( PureNANDSpec *pstPNDSpec , UINT32 dwPageAddr, UINT8 *pu8_DataBuf, UINT8 *pu8_SpareBuf);
UINT32 NFC_WritePage_FreeRun( PureNANDSpec *pstPNDSpec , UINT32 dwPageAddr, UINT8 *dwBuffer, UINT8 *dwExtraBuffer);
UINT32 NFC_ReadPage(PureNANDSpec *pstPNDSpec, UINT32 dwPageAddr, UINT8 *pu8_DataBuf, UINT8 *pu8_SpareBuf);
UINT32 NFC_EraseBlock(PureNANDSpec *pstPNDSpec, UINT32 dwPageAddr);
UINT32 NFC_WritePage(PureNANDSpec *pstPNDSpec, UINT32 dwPageAddr, UINT8 *pu8_DataBuf, UINT8 *pu8_SpareBuf);
...
UINT32  NFC_GetECCBits(UINT32 subpage_cnt)
{
	...
	UINT32 i;
	UINT32 Total_ECN = 0;
	// It has been modify form 683 Macoto 0714
	...
	for ( i=0; i<subpage_cnt; i++ ){
		//       FSR_OAM_DbgMsg("SubPage %d ECN %d\n", i, ECN[i]);
		Total_ECN += ECN[i];
	}

	return Total_ECN;        
}
EXPORT_SYMBOL(NFC_GetECCBits);
...
UINT32 NFC_IRQInit(UINT32 INT_FLAG)
{
	if(gNFC_IRQInit == 0)
	{
		if(request_irq (NFC_558_INT ,NFC_IRQhandler, IRQF_DISABLED, "NFC", NULL))
		{
	...
	BIT_CLEAR(REG_IRQ_MASK, NFC_558_INT_BIT);		//clear mask
	BIT_CLEAR(REG_IRQ_TYPE, NFC_558_INT_BIT);		//set to level trigger
